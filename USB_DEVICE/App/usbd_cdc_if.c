...
extern volatile uint8_t  g_capture_request;
extern volatile uint16_t g_trigger_width;
...
static int8_t CDC_Receive_HS(uint8_t* Buf, uint32_t *Len)
{
  ...
  for (uint32_t i = 0; i < n; i++)
  {
    ...
    switch (c)
    {
      case 's':                       /* start a line capture */
        g_capture_request = 1;
        break;

      case 'r': case 'g': case 'b':
      case 'w': case 'o':             /* LED control */
        CIS_SetLED(c);
        break;
      ...
      case 'd':                       /* set trigger width: 'd' + lo + hi.
        ...
        if (i + 2 < n)
        {
          ...
          i += 2;                     /* consume the two width bytes */
        }
        break;
  ...
  /* Re-arm reception for the next packet. */
